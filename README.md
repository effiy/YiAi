...
- 🔗 **MCP 服务** - 基于 fastapi-mcp 自动暴露 API 为 Model Context Protocol
- 📊 **State Store** - 结构化状态记录 CRUD，支持 CLI 查询和 fire-and-forget 记录
- 🔍 **Observer Reliability** - 请求限流、尾部采样、执行沙箱、重入守卫
- 🛡️ **安全与认证** - 可选的令牌认证，多层安全防护，统一的异常处理
...
| MCP 服务 | 🔗 fastapi-mcp | FastAPI MCP 服务器，自动暴露 API 为 Model Context Protocol |
...
| CLI 工具 | ⌨️ typer + rich | CLI 框架和终端美化，支持表格/JSON/CSV 输出 |
| 重试机制 | 🔄 tenacity | 通用重试库，支持多种重试策略（预留，当前未使用） |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| [未发布] | - | 新增 State Store、Observer Reliability、CLI 工具；文档重构 |
