...
import { genColumns, selectFilterData } from "@/views/proTable/useSelectFilter/metadata";
// ProTable 实例
...
watch(
  ...
  () => proTable.value?.radio && ElMessage.success(`选中 key 为【${proTable.value?.radio}】的数据`)
);
...
const resetPass = async (params: User.ResUserList) => {
  await useHandleData(resetUserPassWord, { key: params.key }, `重置【${params.username}】用户密码`);
  proTable.value?.getTableList();
};
