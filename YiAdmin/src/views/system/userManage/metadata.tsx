...
export const genColumns = reactive<ColumnProps<User.ResUserList>[]>([
  ...
  { type: "index", label: "#", width: 80 },
  {
    ...
    label: "用户头像",
    isShow: false,
    form: { el: "upload-img", rules: [{ required: true, message: "请上传用户头像" }] }
  },
  {
    prop: "photo",
    label: "用户照片",
    isShow: false,
    form: { el: "upload-imgs", rules: [{ required: true, message: "请上传用户照片" }] }
  },
  {
    prop: "username",
    label: "用户姓名",
    width: 120,
    form: { el: "el-input", rules: [{ required: true, message: "请填写用户姓名" }] }
  },
  {
    prop: "gender",
    label: "性别",
    ...
    sortable: true,
    enum: genderType,
    form: { el: "el-select", props: { filterable: true }, rules: [{ required: true, message: "请选择性别" }] }
  },
