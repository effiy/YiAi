...
import { kEnvs } from "../utils/config";
...
class OpenAIClient {
  ...
  private _init() {
    this.deployment = (kEnvs as any).AZURE_OPENAI_DEPLOYMENT;
    if (!this._client) {
      this._client = (kEnvs as any).AZURE_OPENAI_API_KEY
        ? new AzureOpenAI({
  ...
  async chatStream(
    ...
    let {
      ...
      trace = false,
      model = this.deployment ?? kEnvs.OPENAI_MODEL ?? "gpt-4o",
      enableSearch = (kEnvs as any).QWEN_ENABLE_SEARCH,
    } = options;
    if (trace && this.traceInput) {
      this._logger.log(
        `🔥 onAskAI\n🤖️ System: ${system ?? "None"}\n😊 User: ${user}`.trim()
