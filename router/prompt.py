...
@router.get("/")
async def simple_ollama_prompt(
    fromSystem: Optional[str] = Query(default="你是一个有用的AI助手。", description="系统提示词"),
    fromUser: Optional[str] = Query(default="", description="用户消息"),
    model: Optional[str] = Query(default="deepseek-r1:32b", description="模型名称")
):
    """
    最简单的Ollama请求：仅支持文本输入和文本输出，不支持多模态。无流式返回。
    """
    # 默认值
    system_prompt = fromSystem or "你是一个有用的AI助手。"
    user_content = fromUser or ""
    ...
    try:
        ...
        # Ollama v0.1.29+ 返回 dict，内容在 response["message"]["content"]
